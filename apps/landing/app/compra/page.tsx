...
export default function CompraPage() {
  ...
  const loadTables = useCallback(async (eventId?: string) => {
    ...
    try {
      ...
      const loadedTables = Array.isArray(data?.tables) ? data.tables : [];
      setTables(loadedTables);
      const firstAvailable = loadedTables.find((t: any) => !t.is_reserved) || loadedTables[0];
      ...
      const resolvedEventId = eventId || firstAvailable?.event_id || eventRel?.id || "";
      if (resolvedEventId) {
        setSelectedEventId((prev) => prev || resolvedEventId);
      }
    } catch (_err) {
      setTables([]);
      setSelected("");
    ...
    fetch("/api/events", { cache: "no-store" })
      ...
      .then((data) => {
        ...
        if (events.length > 0) {
          ...
          setTicketEventId((prev) => prev || defaultEventId);
          setSelectedEventId((prev) => prev || defaultEventId);
        }
      })
  ...
  useEffect(() => {
    loadTables(selectedEventId || undefined);
  }, [selectedEventId, loadTables]);
