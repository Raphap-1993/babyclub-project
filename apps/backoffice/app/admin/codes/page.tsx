...
async function getEvents(): Promise<Option[]> {
  ...
  const { data } = await applyNotDeleted(
    supabase.from("events").select("id,name,starts_at").order("starts_at", { ascending: false }).limit(200)
  );
...
async function getPromoters(): Promise<Option[]> {
  ...
  const { data } = await applyNotDeleted(
    supabase.from("promoters").select("id,code").order("created_at", { ascending: true }).limit(500)
  );
