...
// Helper: Resolve organizer_id from first organizer or default
async function resolveDefaultOrganizerId(supabase: any): Promise<string | null> {
  const { data, error } = await supabase.from("organizers").select("id").limit(1).maybeSingle();
  return !error && data?.id ? data.id : null;
}

async function getTables(params: { page: number; pageSize: number; organizerId?: string }): Promise<{ tables: TableRow[]; total: number; error?: string }> {
  if (!supabaseUrl || !supabaseServiceKey) return { tables: [], total: 0, error: "Falta configuración de Supabase" };
  const supabase = createClient(supabaseUrl, supabaseServiceKey, {
    auth: { autoRefreshToken: false, persistSession: false },
  });
  
  // Resolve organizer_id for multi-org isolation
  const organizerId = params.organizerId || (await resolveDefaultOrganizerId(supabase));
  if (!organizerId) {
    return { tables: [], total: 0, error: "No organizer found" };
  }
  
  const start = (params.page - 1) * params.pageSize;
  const end = start + params.pageSize - 1;
  
  // CRITICAL: Filter by organizer_id to prevent data leakage
