...
describe("GET /api/admin/reports/export", () => {
  ...
  it("event_attendance: cae a fallback cuando scan_logs no tiene deleted_at", async () => {
    ...
    const req = {
      nextUrl: new URL("http://localhost/api/admin/reports/export?report=event_attendance&format=json"),
    ...
    expect(scanCalls[0]?.filters?.some((f) => f.type === "is" && f.args[0] === "deleted_at")).toBe(true);
    expect(scanCalls[1]?.filters?.some((f) => f.type === "is" && f.args[0] === "deleted_at")).toBe(false);
  });
  ...
  it("promoter_performance: funciona con fallback de scan_logs y calcula métricas", async () => {
    ...
    const req = {
      nextUrl: new URL("http://localhost/api/admin/reports/export?report=promoter_performance&format=json"),
      headers: new Headers({ Authorization: "Bearer token-123" }),
    } as any;
    const res = await GET(req);
    const payload = await res.json();
  ...
  it("event_sales: agrega pagos pagados por evento", async () => {
    ...
    const req = {
      nextUrl: new URL("http://localhost/api/admin/reports/export?report=event_sales&format=json"),
      headers: new Headers({ Authorization: "Bearer token-123" }),
    } as any;
    const res = await GET(req);
    const payload = await res.json();
