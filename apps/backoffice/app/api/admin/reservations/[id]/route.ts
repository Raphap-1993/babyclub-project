...
export async function GET(
  ...
  const [tableData, eventData, conflictingTickets, promoterData] =
    await Promise.all([
      data.table_id
        ? supabase
            .from("tables")
  ...
  const reservation = {
    ...
    ticket_quantity: resolveReservationTicketQuantity({
      ...
      codesCount: Array.isArray(data.codes) ? data.codes.length : 0,
      minimum: 1,
    }),
    total_ticket_units:
...
export async function PATCH(
  ...
  if (
    !status ||
    !["pending", "approved", "rejected", "confirmed"].includes(status)
  ) {
    return NextResponse.json(
