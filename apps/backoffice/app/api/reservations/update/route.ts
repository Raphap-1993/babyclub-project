...
export async function POST(req: NextRequest) {
  ...
  const document =
    typeof body?.document === "string" ? body.document : undefined;
  ...
  const resolvedFullName =
    full_name?.trim() || (reservation as any).full_name || "";
  const resolvedEmail =
    email === "" ? "" : (email ?? ((reservation as any).email || ""));
  const resolvedPhone =
    phone === "" ? "" : (phone ?? ((reservation as any).phone || ""));
  const resolvedDocType = (doc_type ||
    (reservation as any).doc_type ||
    "dni") as any;
  const resolvedDocument =
    document ?? (reservation as any).document ?? (reservation as any).dni ?? "";

  const updateData: Record<string, any> = {};
  if (status && ["pending", "approved", "rejected"].includes(status))
    updateData.status = status;
  if (full_name !== undefined && resolvedFullName.length > 0)
    updateData.full_name = resolvedFullName;
  if (email !== undefined) {
  ...
  if (updateData.status === "approved") {
    ...
    if (!eventId) {
      return NextResponse.json(
        {
          success: false,
          error: "Mesa sin evento asignado; no se generó ticket/QR.",
          trace,
        },
        { status: 400 },
      );
    ...
    try {
      ...
      if (ticketIds.length === 0) {
        return NextResponse.json(
          {
            success: false,
            error: "No se pudieron generar tickets para la aprobación",
            trace,
          },
          { status: 500 },
        );
      ...
      */
      ...
      try {
        ...
        emailSent = true;
        trace.push(
          `approvalEmailSent:tickets=${ticketIds.length},codes=${codesForEmail.length}`,
        );
      } catch (err: any) {
        emailError =
  ...
  if (updateData.status === "rejected") {
    ...
    if (codeRowsByReservation.error) {
      trace.push(
        `error_loading_reservation_codes:${codeRowsByReservation.error.message}`,
      );
      cancellationWarnings.push(
        `codes_by_reservation:${codeRowsByReservation.error.message}`,
      );
    }
    if (codeRowsByLegacyList.error) {
      trace.push(
        `error_loading_legacy_codes:${codeRowsByLegacyList.error.message}`,
      );
      cancellationWarnings.push(
        `codes_by_legacy_list:${codeRowsByLegacyList.error.message}`,
      );
    }

    const codeIds = uniqueStrings([
      ...((codeRowsByReservation.data || []).map(
        (row: any) => row.id,
      ) as string[]),
      ...((codeRowsByLegacyList.data || []).map(
        (row: any) => row.id,
      ) as string[]),
    ]);
    ...
    if (codeIds.length > 0) {
      ...
      if (codesDeactivateError) {
        ...
        cancellationErrors.push(
          `deactivate_codes:${codesDeactivateError.message}`,
        );
      } else {
    ...
    if (codeIds.length > 0) {
      const { data: ticketsByCodes, error: ticketsByCodesError } =
        await supabase
          .from("tickets")
          .select("id")
          .in("code_id", codeIds)
      ...
      if (ticketsByCodesError) {
        trace.push(
          `error_loading_tickets_by_codes:${ticketsByCodesError.message}`,
        );
        cancellationWarnings.push(
          `tickets_by_codes:${ticketsByCodesError.message}`,
        );
      } else {
        ticketIds.push(
    ...
    const { data: ticketsByReservation, error: ticketsByReservationError } =
      await supabase
        .from("tickets")
        .select("id")
        .eq("table_reservation_id", id)
    ...
    if (ticketsByReservationError) {
      trace.push(
        `error_loading_tickets_by_reservation:${ticketsByReservationError.message}`,
      );
      cancellationWarnings.push(
        `tickets_by_reservation:${ticketsByReservationError.message}`,
      );
    } else {
      ticketIds.push(
        ...uniqueStrings(
          (ticketsByReservation || []).map((row: any) => row.id),
        ),
      );
    }
    ...
    const reservationTableId =
      (reservation as any).table_id || tableRel?.id || null;
    ...
    if (reservationTableId && reservationEventId) {
      const { data: ticketsByTableEvent, error: ticketsByTableEventError } =
        await supabase
          .from("tickets")
          .select("id")
      ...
      if (ticketsByTableEventError) {
        trace.push(
          `error_loading_tickets_by_table_event:${ticketsByTableEventError.message}`,
        );
        cancellationWarnings.push(
          `tickets_by_table_event:${ticketsByTableEventError.message}`,
        );
      } else {
        ticketIds.push(
          ...uniqueStrings(
            (ticketsByTableEvent || []).map((row: any) => row.id),
          ),
        );
      }
    }
    ...
    if (uniqueTicketIds.length > 0) {
      ...
      if (deactivateTicketsError) {
        trace.push(
          `error_deactivating_tickets:${deactivateTicketsError.message}`,
        );
        cancellationErrors.push(
          `deactivate_tickets:${deactivateTicketsError.message}`,
        );
      } else {
        ...
        if (markCancelledError) {
          trace.push(
            `warning_mark_tickets_cancelled:${markCancelledError.message}`,
          );
          if (!isMissingColumnError(markCancelledError)) {
            cancellationWarnings.push(
              `mark_tickets_cancelled:${markCancelledError.message}`,
            );
          }
        }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "La reserva se anuló, pero falló la cancelación de tickets/códigos vinculados",
          trace,
          cancellationErrors,
    ...
    if (resolvedEmail) {
      try {
        ...
        emailError =
          err?.message || "No se pudo enviar el correo de cancelación";
        trace.push(`cancellationEmailError:${emailError}`);
        console.error("[reservations/update] cancellation email error", {
          id,
          trace,
          err,
        });
