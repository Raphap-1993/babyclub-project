-- Code batches and RPC for bulk generation with auditing
...
create table if not exists public.code_batches (
  ...
  type text not null check (type in ('courtesy','promoter','table')),
...
-- 2) Relación con codes + índices
alter table public.codes add column if not exists batch_id uuid references public.code_batches(id);

create index if not exists codes_batch_idx on public.codes(batch_id);
create index if not exists codes_event_type_active_idx on public.codes(event_id, type, is_active);
create index if not exists codes_promoter_event_idx on public.codes(promoter_id, event_id);
...
begin
  if not exists (
    ...
    where policyname = 'code_batches_staff_rw' and schemaname = 'public' and tablename = 'code_batches'
  ) then
    create policy code_batches_staff_rw on public.code_batches
      for all
  ...
  if not exists (
    select 1 from pg_policies
    where policyname = 'codes_staff_rw' and schemaname = 'public' and tablename = 'codes'
  ) then
    create policy codes_staff_rw on public.codes
      for all
      using (exists (select 1 from public.staff s where s.auth_user_id = auth.uid() and coalesce(s.is_active, true)))
      with check (exists (select 1 from public.staff s where s.auth_user_id = auth.uid() and coalesce(s.is_active, true)));
  end if;
...
-- 4) RPC para generar lotes de códigos
  ...
  for i in 1..v_quantity loop
    ...
    loop
      ...
      begin
        ...
        values (v_code, p_event_id, p_promoter_id, v_type, true, v_max_uses, 0, v_expiration, v_batch_id)
