...
export async function GET(req: NextRequest) {
  ...
  if (!dni && !email && !phone) {
    return NextResponse.json({ success: false, error: "dni, email o phone requerido" }, { status: 400 });
  }

  let query = supabase.from("persons").select("id,dni,first_name,last_name,email,phone").limit(1);
  const ors: string[] = [];
